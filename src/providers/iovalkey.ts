...
/**
 * Cache Provider implementation using the iovalkey library.
 */
export class IovalkeyCacheProvider implements CacheProvider {
  private redis: Redis;
  ...
  async getJson<T = unknown>(key: string): Promise<T | null> {
    try {
      ...
      console.error(`iovalkey: Error getting JSON for key ${key}:`, error);
      return null;
    }
  }
  ...
  async setJson<T = unknown>(
    ...
    try {
      ...
      console.error(`iovalkey: Error setting JSON for key ${key}:`, error);
      throw error;
    }
  }
